...
`make package CONFIG=Release` writes `dist/VGBBoardAlways-v<version>.zip` and the matching `dist/update.json` feed. Packaging validates that the project version, `PluginVersion`, the version compiled into the packaged assembly and the release tag all agree, and that the metadata sidecar stays within the Mod API's documented field limits. The compiled version is read from the assembly's own Win32 version resource rather than scanned from raw text, because referenced type and assembly strings also contain versions. `python3 -m unittest test_package` in `tools/` covers that gate, including rejection of a stale or foreign DLL dropped into the build output and of a reference version standing in for our own. Publication uploads those exact assets to the tagged release.
